# Certificates Create Body

Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/ruby/x-redirect/JTI0bSUyRkNlcnRpZmljYXRlc0NyZWF0ZUJvZHk


# Data Type

`LetSEncryptCertificateRequest | CustomCertificateRequest`


# Cases

| Type |
|  --- |
| [`LetSEncryptCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/ruby/models/structures/let-s-encrypt-certificate-request.md) |
| [`CustomCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/ruby/models/structures/custom-certificate-request.md) |


# LetSEncryptCertificateRequest

## Initialization Code

### Example

```ruby
value = LetSEncryptCertificateRequest.new(
  name: 'web-cert-01',
  dns_names: [
    'www.example.com',
    'example.com'
  ],
  type: Type4::LETS_ENCRYPT
)
```


# CustomCertificateRequest

## Initialization Code

### Example

```ruby
value = CustomCertificateRequest.new(
  name: 'web-cert-01',
  leaf_certificate: '-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----',
  private_key: '-----BEGIN PRIVATE KEY-----\\neTuK2xNR0PIM8OI7pRpgyj1I\n-----END PRIVATE KEY-----',
  type: Type4::LETS_ENCRYPT,
  certificate_chain: '-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----'
)
```



