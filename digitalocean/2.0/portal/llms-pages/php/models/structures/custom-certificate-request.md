# Custom Certificate Request

Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/php/x-redirect/JTI0bSUyRkN1c3RvbSUyNTIwQ2VydGlmaWNhdGUlMjUyMFJlcXVlc3Q

*This model accepts additional fields of type [array](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/php/models/structures/object.md).*


# Class Name

`CustomCertificateRequest`


# Fields

| Name | Type | Tags | Description | Getter | Setter |
|  --- | --- | --- | --- | --- | --- |
| `name` | `string` | Required | A unique human-readable name referring to a certificate. | getName(): string | setName(string name): void |
| `type` | [`?string(Type4)`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/php/models/enumerations/type-4.md) | Optional | A string representing the type of the certificate. The value will be `custom` for a user-uploaded certificate or `lets_encrypt` for one automatically generated with Let's Encrypt. | getType(): ?string | setType(?string type): void |
| `certificateChain` | `?string` | Optional | The full PEM-formatted trust chain between the certificate authority's certificate and your domain's SSL certificate. | getCertificateChain(): ?string | setCertificateChain(?string certificateChain): void |
| `leafCertificate` | `string` | Required | The contents of a PEM-formatted public SSL certificate. | getLeafCertificate(): string | setLeafCertificate(string leafCertificate): void |
| `privateKey` | `string` | Required | The contents of a PEM-formatted private-key corresponding to the SSL certificate. | getPrivateKey(): string | setPrivateKey(string privateKey): void |
| `additionalProperties` | [`array<string, array>`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/php/models/structures/object.md) | Optional | - | findAdditionalProperty(string key): array | additionalProperty(string key, array value): void |


# Example

```php
use DigitalOceanApiLib\Models\Builders\CustomCertificateRequestBuilder;
use DigitalOceanApiLib\Models\Type4;
use DigitalOceanApiLib\ApiHelper;

$customCertificateRequest = CustomCertificateRequestBuilder::init(
    'web-cert-01',
    '-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----',
    '-----BEGIN PRIVATE KEY-----

eTuK2xNR0PIM8OI7pRpgyj1I
-----END PRIVATE KEY-----'
)
    ->type(Type4::LETS_ENCRYPT)
    ->certificateChain('-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----')
    ->additionalProperty('exampleAdditionalProperty', ApiHelper::deserialize('{"key1":"val1","key2":"val2"}'))
    ->build();
```



