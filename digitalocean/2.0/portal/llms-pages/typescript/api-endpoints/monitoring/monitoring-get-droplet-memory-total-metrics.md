...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRk1vbml0b3JpbmclMkZtb25pdG9yaW5nX2dldF9kcm9wbGV0TWVtb3J5VG90YWxNZXRyaWNz

To retrieve total memory metrics for a given droplet, send a GET request to `/v2/monitoring/metrics/droplet/memory_total`.

```ts
async monitoringGetDropletMemoryTotalMetrics(
  hostId: string,
  start: string,
  end: string,
...
try {
  const response = await monitoringApi.monitoringGetDropletMemoryTotalMetrics(
    hostId,
    start,
    end
  );
