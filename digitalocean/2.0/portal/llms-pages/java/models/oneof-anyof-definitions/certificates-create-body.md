# Certificates Create Body

Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/java/x-redirect/JTI0bSUyRkNlcnRpZmljYXRlc0NyZWF0ZUJvZHk


# Class Name

`CertificatesCreateBody`


# Cases

| Type | Factory Method |
|  --- | --- |
| [`LetSEncryptCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/java/models/structures/let-s-encrypt-certificate-request.md) | CertificatesCreateBody.fromLetSEncryptCertificateRequest(LetSEncryptCertificateRequest letSEncryptCertificateRequest) |
| [`CustomCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/java/models/structures/custom-certificate-request.md) | CertificatesCreateBody.fromCustomCertificateRequest(CustomCertificateRequest customCertificateRequest) |


# LetSEncryptCertificateRequest

## Initialization Code

### Example

```java
CertificatesCreateBody.fromLetSEncryptCertificateRequest(
        new LetSEncryptCertificateRequest.Builder(
            "web-cert-01",
            Arrays.asList(
                "www.example.com",
                "example.com"
            )
        )
        .type(Type4.LETS_ENCRYPT)
        .build()
    )
```


# CustomCertificateRequest

## Initialization Code

### Example

```java
CertificatesCreateBody.fromCustomCertificateRequest(
        new CustomCertificateRequest.Builder(
            "web-cert-01",
            "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
            "-----BEGIN PRIVATE KEY-----\\neTuK2xNR0PIM8OI7pRpgyj1I\n-----END PRIVATE KEY-----"
        )
        .type(Type4.LETS_ENCRYPT)
        .certificateChain("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----")
        .build()
    )
```



