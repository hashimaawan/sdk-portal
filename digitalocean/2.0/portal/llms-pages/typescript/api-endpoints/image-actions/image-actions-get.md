...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRkltYWdlJTI1MjBBY3Rpb25zJTJGaW1hZ2VBY3Rpb25zX2dldA

To retrieve the status of an image action, send a GET request to `/v2/images/$IMAGE_ID/actions/$IMAGE_ACTION_ID`.

```ts
async imageActionsGet(
  imageId: number,
  actionId: number,
...
try {
  const response = await imageActionsApi.imageActionsGet(
    imageId,
    actionId
  );
