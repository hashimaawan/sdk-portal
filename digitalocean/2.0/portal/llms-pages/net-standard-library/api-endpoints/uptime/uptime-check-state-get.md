...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/net-standard-library/x-redirect/JTI0ZSUyRlVwdGltZSUyRnVwdGltZV9jaGVja19zdGF0ZV9nZXQ

To show information about an existing check's state, send a GET request to `/v2/uptime/checks/$CHECK_ID/state`.

```csharp
UptimeCheckStateGetAsync(
    Guid checkId)
```
...
try
{
    ApiResponse<V2UptimeChecksStateResponse> result = await uptimeApi.UptimeCheckStateGetAsync(checkId);
}
