...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/php/x-redirect/JTI0ZSUyRkJsb2NrJTI1MjBTdG9yYWdlJTJGdm9sdW1lU25hcHNob3RzX2xpc3Q

To retrieve the snapshots that have been created from a volume, send a GET request to `/v2/volumes/$VOLUME_ID/snapshots`.

```php
function volumeSnapshotsList(string $volumeId, ?int $perPage = 20, ?int $page = 1): ApiResponse
```
...
$apiResponse = $blockStorageApi->volumeSnapshotsList(
    $volumeId,
    $perPage,
    $page
);
...
if ($apiResponse->isSuccess()) {
    echo 'V2VolumesSnapshotsResponse1:';
