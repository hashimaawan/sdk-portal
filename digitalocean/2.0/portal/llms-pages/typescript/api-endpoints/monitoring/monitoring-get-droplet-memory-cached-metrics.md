...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRk1vbml0b3JpbmclMkZtb25pdG9yaW5nX2dldF9kcm9wbGV0TWVtb3J5Q2FjaGVkTWV0cmljcw

To retrieve cached memory metrics for a given droplet, send a GET request to `/v2/monitoring/metrics/droplet/memory_cached`.

```ts
async monitoringGetDropletMemoryCachedMetrics(
  hostId: string,
  start: string,
  end: string,
...
try {
  const response = await monitoringApi.monitoringGetDropletMemoryCachedMetrics(
    hostId,
    start,
    end
  );
