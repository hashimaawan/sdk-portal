# Certificates Create Body

Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/go/x-redirect/JTI0bSUyRkNlcnRpZmljYXRlc0NyZWF0ZUJvZHk


# Class Name

`CertificatesCreateBody`


# Cases

| Type | Factory Method |
|  --- | --- |
| [`models.LetSEncryptCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/go/models/structures/let-s-encrypt-certificate-request.md) | models.CertificatesCreateBodyContainer.FromLetSEncryptCertificateRequest(models.LetSEncryptCertificateRequest letSEncryptCertificateRequest) |
| [`models.CustomCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/go/models/structures/custom-certificate-request.md) | models.CertificatesCreateBodyContainer.FromCustomCertificateRequest(models.CustomCertificateRequest customCertificateRequest) |


# models.LetSEncryptCertificateRequest

## Initialization Code

### Example

```go
value := models.CertificatesCreateBodyContainer.FromLetSEncryptCertificateRequest(models.LetSEncryptCertificateRequest{
    Name:                  "web-cert-01",
    Type:                  models.ToPointer(models.Type4_LetsEncrypt),
    DnsNames:              []string{
        "www.example.com",
        "example.com",
    },
})
```


# models.CustomCertificateRequest

## Initialization Code

### Example

```go
value := models.CertificatesCreateBodyContainer.FromCustomCertificateRequest(models.CustomCertificateRequest{
    Name:                  "web-cert-01",
    Type:                  models.ToPointer(models.Type4_LetsEncrypt),
    CertificateChain:      models.ToPointer("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"),
    LeafCertificate:       "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
    PrivateKey:            "-----BEGIN PRIVATE KEY-----\\neTuK2xNR0PIM8OI7pRpgyj1I\n-----END PRIVATE KEY-----",
})
```



