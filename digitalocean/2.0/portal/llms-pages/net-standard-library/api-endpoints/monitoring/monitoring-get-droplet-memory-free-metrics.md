...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/net-standard-library/x-redirect/JTI0ZSUyRk1vbml0b3JpbmclMkZtb25pdG9yaW5nX2dldF9kcm9wbGV0TWVtb3J5RnJlZU1ldHJpY3M

To retrieve free memory metrics for a given droplet, send a GET request to `/v2/monitoring/metrics/droplet/memory_free`.

```csharp
MonitoringGetDropletMemoryFreeMetricsAsync(
    string hostId,
    string start,
    string end)
```
...
try
{
    ApiResponse<V2MonitoringMetricsDropletBandwidthResponse> result = await monitoringApi.MonitoringGetDropletMemoryFreeMetricsAsync(
        hostId,
        start,
        end
    );
