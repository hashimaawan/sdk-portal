...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRk1vbml0b3JpbmclMkZtb25pdG9yaW5nX2dldF9kcm9wbGV0TG9hZDFNZXRyaWNz

To retrieve 1 minute load average metrics for a given droplet, send a GET request to `/v2/monitoring/metrics/droplet/load_1`.

```ts
async monitoringGetDropletLoad1Metrics(
  hostId: string,
  start: string,
  end: string,
...
try {
  const response = await monitoringApi.monitoringGetDropletLoad1Metrics(
    hostId,
    start,
    end
  );
