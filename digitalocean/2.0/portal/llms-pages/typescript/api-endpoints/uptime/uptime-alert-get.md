...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRlVwdGltZSUyRnVwdGltZV9hbGVydF9nZXQ

To show information about an existing alert, send a GET request to `/v2/uptime/checks/$CHECK_ID/alerts/$ALERT_ID`.

```ts
async uptimeAlertGet(
  checkId: string,
  alertId: string,
...
try {
  const response = await uptimeApi.uptimeAlertGet(
    checkId,
    alertId
  );
