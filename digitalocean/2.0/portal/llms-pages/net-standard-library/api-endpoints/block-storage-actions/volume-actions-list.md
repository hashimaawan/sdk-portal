...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/net-standard-library/x-redirect/JTI0ZSUyRkJsb2NrJTI1MjBTdG9yYWdlJTI1MjBBY3Rpb25zJTJGdm9sdW1lQWN0aW9uc19saXN0

To retrieve all actions that have been executed on a volume, send a GET request to `/v2/volumes/$VOLUME_ID/actions`.

```csharp
VolumeActionsListAsync(
    Guid volumeId,
    int? perPage = 20,
    int? page = 1)
```
...
try
{
    ApiResponse<V2VolumesActionsResponse1> result = await blockStorageActionsApi.VolumeActionsListAsync(
        volumeId,
        perPage,
        page
    );
