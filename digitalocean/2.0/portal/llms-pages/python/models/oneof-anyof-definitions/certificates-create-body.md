# Certificates Create Body

Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/python/x-redirect/JTI0bSUyRkNlcnRpZmljYXRlc0NyZWF0ZUJvZHk


# Data Type

`LetSEncryptCertificateRequest | CustomCertificateRequest`


# Cases

| Type |
|  --- |
| [`LetSEncryptCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/python/models/structures/let-s-encrypt-certificate-request.md) |
| [`CustomCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/python/models/structures/custom-certificate-request.md) |


# LetSEncryptCertificateRequest

## Initialization Code

### Example

```python
value = LetSEncryptCertificateRequest(
    name='web-cert-01',
    dns_names=[
        'www.example.com',
        'example.com'
    ],
    mtype=Type4.LETS_ENCRYPT
)
```


# CustomCertificateRequest

## Initialization Code

### Example

```python
value = CustomCertificateRequest(
    name='web-cert-01',
    leaf_certificate='-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----',
    private_key='-----BEGIN PRIVATE KEY-----\\neTuK2xNR0PIM8OI7pRpgyj1I\n-----END PRIVATE KEY-----',
    mtype=Type4.LETS_ENCRYPT,
    certificate_chain='-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----'
)
```



