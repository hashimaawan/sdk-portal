...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/net-standard-library/x-redirect/JTI0ZSUyRkNETiUyNTIwRW5kcG9pbnRzJTJGY2RuX2xpc3RfZW5kcG9pbnRz

To list all of the CDN endpoints available on your account, send a GET request to `/v2/cdn/endpoints`.

```csharp
CdnListEndpointsAsync(
    int? perPage = 20,
    int? page = 1)
```
...
try
{
    ApiResponse<V2CdnEndpointsResponse> result = await cdnEndpointsApi.CdnListEndpointsAsync(
        perPage,
        page
    );
}
