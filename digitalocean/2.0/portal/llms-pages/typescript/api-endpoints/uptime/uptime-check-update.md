...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRlVwdGltZSUyRnVwdGltZV9jaGVja191cGRhdGU

To update the settings of an Uptime check, send a PUT request to `/v2/uptime/checks/$CHECK_ID`.

```ts
async uptimeCheckUpdate(
  checkId: string,
...
try {
  const response = await uptimeApi.uptimeCheckUpdate(
    checkId,
    body
  );
