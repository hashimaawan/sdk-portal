...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/net-standard-library/x-redirect/JTI0ZSUyRk1vbml0b3JpbmclMkZtb25pdG9yaW5nX3VwZGF0ZV9hbGVydFBvbGljeQ

To update en existing policy, send a PUT request to `v2/monitoring/alerts/{alert_uuid}`.

```csharp
MonitoringUpdateAlertPolicyAsync(
    string alertUuid,
...
try
{
    ApiResponse<V2MonitoringAlertsResponse1> result = await monitoringApi.MonitoringUpdateAlertPolicyAsync(
        alertUuid,
        body
    );
}
