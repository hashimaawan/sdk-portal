...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/net-standard-library/x-redirect/JTI0ZSUyRkRvbWFpbiUyNTIwUmVjb3JkcyUyRmRvbWFpbnNfZ2V0X3JlY29yZA

To retrieve a specific domain record, send a GET request to `/v2/domains/$DOMAIN_NAME/records/$RECORD_ID`.

```csharp
DomainsGetRecordAsync(
    string domainName,
    int domainRecordId)
```
...
try
{
    ApiResponse<V2DomainsRecordsResponse1> result = await domainRecordsApi.DomainsGetRecordAsync(
        domainName,
        domainRecordId
    );
}
