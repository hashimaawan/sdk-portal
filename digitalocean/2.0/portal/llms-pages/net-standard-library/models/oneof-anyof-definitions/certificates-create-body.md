# Certificates Create Body

Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/net-standard-library/x-redirect/JTI0bSUyRkNlcnRpZmljYXRlc0NyZWF0ZUJvZHk


# Class Name

`CertificatesCreateBody`


# Cases

| Type | Factory Method |
|  --- | --- |
| [`LetSEncryptCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/net-standard-library/models/structures/let-s-encrypt-certificate-request.md) | CertificatesCreateBody.FromLetSEncryptCertificateRequest(LetSEncryptCertificateRequest letSEncryptCertificateRequest) |
| [`CustomCertificateRequest`](https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/llms-pages/net-standard-library/models/structures/custom-certificate-request.md) | CertificatesCreateBody.FromCustomCertificateRequest(CustomCertificateRequest customCertificateRequest) |


# LetSEncryptCertificateRequest

## Initialization Code

### Example

```csharp
CertificatesCreateBody value = CertificatesCreateBody.FromLetSEncryptCertificateRequest(
    new LetSEncryptCertificateRequest
    {
        Name = "web-cert-01",
        DnsNames = new List<string>
        {
            "www.example.com",
            "example.com",
        },
        Type = Type4.LetsEncrypt,
    }
);
```


# CustomCertificateRequest

## Initialization Code

### Example

```csharp
CertificatesCreateBody value = CertificatesCreateBody.FromCustomCertificateRequest(
    new CustomCertificateRequest
    {
        Name = "web-cert-01",
        LeafCertificate = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
        PrivateKey = "-----BEGIN PRIVATE KEY-----\\neTuK2xNR0PIM8OI7pRpgyj1I\n-----END PRIVATE KEY-----",
        Type = Type4.LetsEncrypt,
        CertificateChain = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
    }
);
```



