...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRlVwdGltZSUyRnVwdGltZV9jaGVja19nZXQ

To show information about an existing check, send a GET request to `/v2/uptime/checks/$CHECK_ID`.

```ts
async uptimeCheckGet(
  checkId: string,
...
try {
  const response = await uptimeApi.uptimeCheckGet(checkId);
