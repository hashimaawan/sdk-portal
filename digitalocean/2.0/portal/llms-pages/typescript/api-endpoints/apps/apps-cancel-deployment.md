...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRkFwcHMlMkZhcHBzX2NhbmNlbF9kZXBsb3ltZW50

Immediately cancel an in-progress deployment.

```ts
async appsCancelDeployment(
  appId: string,
  deploymentId: string,
...
try {
  const response = await appsApi.appsCancelDeployment(
    appId,
    deploymentId
  );
