...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRlVwdGltZSUyRnVwdGltZV9hbGVydF9kZWxldGU

To delete an Uptime alert, send a DELETE request to `/v2/uptime/checks/$CHECK_ID/alerts/$ALERT_ID`. A 204 status
code with no body will be returned in response to a successful request.

```ts
async uptimeAlertDelete(
  checkId: string,
  alertId: string,
...
try {
  const response = await uptimeApi.uptimeAlertDelete(
    checkId,
    alertId
  );
