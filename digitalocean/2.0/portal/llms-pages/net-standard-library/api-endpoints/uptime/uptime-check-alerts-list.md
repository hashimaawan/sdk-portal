...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/net-standard-library/x-redirect/JTI0ZSUyRlVwdGltZSUyRnVwdGltZV9jaGVja19hbGVydHNfbGlzdA

To list all of the alerts for an Uptime check, send a GET request to `/v2/uptime/checks/$CHECK_ID/alerts`.

```csharp
UptimeCheckAlertsListAsync(
    Guid checkId,
    int? perPage = 20,
    int? page = 1)
```
...
try
{
    ApiResponse<V2UptimeChecksAlertsResponse> result = await uptimeApi.UptimeCheckAlertsListAsync(
        checkId,
        perPage,
        page
    );
