...
Source: https://raw.githubusercontent.com/hashimaawan/sdk-portal/main/digitalocean/2.0/portal/#/typescript/x-redirect/JTI0ZSUyRkJpbGxpbmclMkZpbnZvaWNlc19nZXRfcGRmQnlVVUlE

To retrieve a PDF for an invoice, send a GET request to `/v2/customers/my/invoices/$INVOICE_UUID/pdf`.

```ts
async invoicesGetPdfByUuid(
  invoiceUuid: string,
...
try {
  const response = await billingApi.invoicesGetPdfByUuid(invoiceUuid);
